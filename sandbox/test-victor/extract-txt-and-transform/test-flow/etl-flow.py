...
# Task to call the new function that loads Parquet data into BigQuery
@task(retries=3, retry_delay_seconds=60)
def load_to_bigquery():
    cloud_function_url = "https://load-to-bigquery-162771833878.us-central1.run.app"
    response = requests.post(cloud_function_url)
    if response.status_code == 200:
...
def combined_pipeline_flow():
    ...
    transform_result = transform_txt_to_dataframe(wait_for=[schema_result])

    # Step 4: Load Parquet data into BigQuery
    load_to_bigquery(wait_for=[transform_result])
